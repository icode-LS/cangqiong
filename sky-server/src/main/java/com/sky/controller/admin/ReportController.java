...
public class ReportController {
    ...
    @ApiOperation("订单统计")
    public Result<OrderReportVO> getOrdersStatics(@DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate begin,
                                                  @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate end) {
    ...
    @GetMapping("/top10")
    @ApiOperation("查看销量top10")
    public Result<SalesTop10ReportVO> getSalesTop1(@DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate begin,
                                                   @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate end) {
        log.info("查看销量top10" + begin + "  " + end);
        return Result.success(reportService.getSalesTop10(begin, end));
    }

    @GetMapping("/export")
    @ApiOperation("导出Excel报表")
    public void exportExcel(HttpServletResponse httpResponse) {
        log.info("导出Excel报表");
